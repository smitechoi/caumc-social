...
// 2. 새 환자 생성 (등록번호 추가)
export async function createPatient(name, birthDate, language, registrationNumber) {
  const patientId = `${name}_${birthDate.replace(/-/g, '')}`;
  ...
  return withRetry(async () => {
    ...
    if (registrationNumber) {
      const regRef = doc(db, 'registrations', registrationNumber);
      await setDoc(regRef, {
        patientId,
        name,
        birthDate
...
// 7. 등록번호로 환자 조회
export async function getPatientByRegistrationNumber(registrationNumber) {
  return withRetry(async () => {
    // 등록번호로 환자 ID 조회
    const regRef = doc(db, 'registrations', registrationNumber);
